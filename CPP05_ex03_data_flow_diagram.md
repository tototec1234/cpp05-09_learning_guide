...
3. ex03でやることは「署名・実行（ex02までの機能）」や「メモリ管理（`delete`）」の仕組み自体はそのままで、ex03の本質的な差分・追加要素は、オブジェクトの生成を呼び出し側（`main`）の直接 `new` から、生成役である `Intern`（とその内部の表・生成関数）に委譲する点（＝生成の入口を `Intern` にすること）だけです！
